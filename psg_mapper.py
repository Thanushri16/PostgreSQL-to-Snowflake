...
meta = pd.read_excel(path+'\\psg_meta_info.xlsx', 'meta_tables')
meta = meta.apply(lambda x: x.astype(str).str.lower())
meta = meta[['table_name','column_name','data_type','character_maximum_length','numeric_precision','is_nullable','constraint_type','constraint_name']]
...
df = pd.DataFrame(c.fetchall(),columns = ['psg_table_name','psg_column_name','psg_data_type','psg_size','psg_precision','psg_isnullable','psg_constraint_type','psg_constraint_name'])
...
df[['sflake_precision']] = df[['psg_precision']]
df[['sflake_isnullable']] = df[['psg_isnullable']]
df[['sflake_constraint_type']] = df[['psg_constraint_type']]
df[['sflake_constraint_name']] = df[['psg_constraint_name']]
print(df[['psg_column_name','psg_data_type','sflake_column_name','sflake_data_type']].head(15))
...
writer = pd.ExcelWriter(path+'\\psg2snow_src_tgt_mapping.xlsx', engine='xlsxwriter')
export_excel = df.to_excel (writer, sheet_name = "source_target_mapping",index = None, header=True)
df.to_sql('psg2snow_src_tgt_mapping', conn, if_exists='replace', index = False)
print(pd.read_sql('SELECT * FROM psg2snow_src_tgt_mapping', conn))
conn.commit()
conn.close()
...
os.startfile(path+'\\psg2snow_src_tgt_mapping.xlsx')
